...
> 修订说明：
> - V2.2 **单一 LLM 供应商切换为国产 DeepSeek**：单一模型 `deepseek-v4-flash` + 每请求 `reasoning` 开关（reasoner/chat）取代多模型档位；模型接口改为 OpenAI 兼容协议（Function Calling / `response_format` / 自动上下文缓存）。
...
### 1.2 设计原则
- **一套网关，单一大模型**：所有模型调用由统一网关 `ModelGateway` 收敛，智能能力基于**单一 LLM 服务（国产 DeepSeek，`deepseek-v4-flash`）**，业务层不感知实现细节。
- **大脑 + 双手 + 记忆**：LLM 负责推理（大脑），Function Calling 负责执行（双手），向量库+自动上下文缓存负责记忆。
- **能力按思考开关调用**：同一模型上通过 `reasoning` 开关切分——深度推理开思考（reasoner）、批量/成本敏感任务关思考（chat），并充分启用 DeepSeek 自动上下文缓存，保持质量与成本的平衡。
...
| LLM Provider | 单个模型服务封装（本期为 DeepSeek）|
| Function Call | 大模型的函数调用机制，用于执行外部动作 |
| 自动上下文缓存 | DeepSeek 开箱即用的上下文缓存，命中时输入费用大幅降低 |
| reasoning 开关 | 同一模型上切换思考模式：on（reasoner 强推理）/ off（chat 标准任务）|
...
flowchart TB
    ...
    subgraph A[能力层]
        ...
        LLM[DeepSeek<br/>deepseek-v4-flash · reasoning 开关]
        TOOL[Tool 执行器]
    end
...
| 能力层 | ModelGateway + DeepSeek + 函数执行器 | 模型调用与实际动作执行 | 单一供应商，实现细节在网关内部收敛 |
...
| 维度 | 选型 | 说明 |
|---|---|---|
| 服务端框架 | Python FastAPI | 异步，易适配 OpenAI 兼容协议 |
| 统一模型网关 SDK | 自研 `ModelGateway` 封装 | 统一调用参数/成本/重试/缓存 |
| 模型服务 | DeepSeek API（`deepseek-v4-flash`）| 单一 LLM 依赖，支持 Function Calling / reasoning 思考 / 自动缓存 |
...
| 决策编号 | 决策 | 理由 |
|---|---|---|
| ADR-01 | 采用「统一网关 + 单一 DeepSeek」而非多厂商混合 | 简化架构与运维，聚焦产品能力；规避多供应商集成复杂度 |
...
智能能力基于 **DeepSeek（国产）`deepseek-v4-flash`** 单一模型交付。网关按任务类型在同一模型上切换 `reasoning` 开关（reasoner / chat）与调用参数（单一供应商、单一模型，不涉及跨模型路由）。能力约束如下：

| 能力项 | 支持情况 | 用途 |
|---|---|---|
| 函数调用（Function Calling）| 支持 | Agent 双手执行（计划更新、知识写入等）|
| 思考模式（reasoning=on）| 支持 | 深度因果推理（冲突判断、偏误归因）|
| 长上下文 | 支持 | 多轮澄清、长文上下文承载 |
| 自动上下文缓存 | 支持（开箱即用）| 系统提示词/知识摘要自动缓存，命中降本 |
| reasoning 开关 | 同模型切换 | 深度任务开思考（on），机械任务关思考（off）|

> 成本预估：单一模型内通过 reasoning 开关（开思考成本更高）+ 自动上下文缓存 + 批量合并控制成本，具体单价以接入时为准（本文档不固锁价格，避免失真）。

#### 3.1.2 任务类型 → 调用策略

网关按任务类型静态映射**调用策略**（配表驱动，可热更新），各任务在同一模型上设置 `reasoning` 开关与关键参数。缺省策略为"默认对话（reasoning=off）"。

| 任务类型 | 典型业务场景 | 模型 | reasoning | 关键参数 | 说明 |
|---|---|---|---|---|---|
| multi_turn_dialogue | 多轮对话、追问澄清（L1/L3）| deepseek-v4-flash | off | 长上下文 | 承载完整澄清轮次 |
| deep_reasoning | 逻辑分析、语义比对（L2/L5）| deepseek-v4-flash | on | — | 深度推理质量优先 |
| conflict_detection | 主张冲突判断（L2）| deepseek-v4-flash | on | 结构化输出回退解析 | 需因果/逻辑链 |
| plan_generation | 计划生成/重规划（L4）| deepseek-v4-flash | off | 函数调用（Function Calling）| 规划与工具能力 |
| causal_reasoning | 偏误因果归因（L5）| deepseek-v4-flash | on | — | 多因素归因 |
| cognitive_brief | 认知简报/追问生成（L3）| deepseek-v4-flash | off | 提示词模板 | 生成式任务 |
| batch_extraction | 批量核心主张提取（L2/L5 前置）| deepseek-v4-flash | off | JSON 模式 + 自动缓存 | 成本敏感、高吞吐 |
| topic_analysis | 主题分布/深度分析（L3）| deepseek-v4-flash | off | JSON 模式 | 批量计算，轻量足够 |
...
flowchart LR
    Task[任务类型] --> P{策略映射}
    P -->|对话/简报/批量| C[deepseek-v4-flash · reasoning=off]
    P -->|深度/冲突/归因| R[deepseek-v4-flash · reasoning=on]
    C --> Fail{超时/失败?}
    R --> Fail
...
| 中期记忆 | 自动上下文缓存 | 知识库摘要、常用系统提示词，命中降本 |
| 长期记忆 | PostgreSQL + 向量库 | 知识条目、学习计划、历史诊断、用户画像 |

**自动上下文缓存策略**：系统提示词、知识库全量摘要等高频不变内容由 DeepSeek 自动上下文缓存命中，命中后输入费用大幅降低（无需手动管理缓存前缀）。
...
五大功能按智能体主动性从低到高（L1→L5），每个功能采用统一的 **Agent 工作流五步范式：感知 → 推理 → 生成/规划 → 行动 → 交付**。全部推理、生成由统一网关调用 **DeepSeek** 完成。
...
**技术要点**
- 用 DeepSeek 长上下文承载完整多轮澄清对话。
- 向量语义相似度 + 关键词兜底双通道召回。
- 追问问题由 DeepSeek 动态生成，不强依赖预设模板。
...
**技术要点**
- 批量主张提取用 reasoning=off + 自动上下文缓存降本。
- 冲突判断用 DeepSeek reasoning=on 思考模式做深度逻辑推理（质量优先）。
...
**技术要点**：生成任务采用 deepseek-v4-flash 配提示词模板；主题分布分析用 reasoning=off 控制成本；追问保留数据依据，增强信服力。
...
**技术要点**：启用 DeepSeek reasoning=on 思考模式做因果推理；诊断结论建议主动询问是否采纳，采纳后回写计划系统形成闭环。
...
| 双手 | Function 执行器 | 通过 Function Calling 执行持久化动作 |

### 6.2 工具注册表（白名单）

| 工具名 | 动作 | 权限 | 由谁调用 |
|---|---|---|---|
| `create_goal` | 创建学习目标 | 用户确认 | DeepSeek 规划 |
| `update_plan` | 更新学习计划 | 用户确认 | DeepSeek 规划 |
| `create_diagnosis` | 写入诊断报告 | 系统 | DeepSeek/Orchestrator |
...
| 步骤 | 动作 | 说明 |
|---|---|---|
| 1 | 模型请求工具调用 | DeepSeek 输出 function_call |
...
业务层仅面向抽象接口编程，本期实现为 `DeepSeekProvider`，保留未来替换能力。协议方法如下：

| 接口方法 | 入参 | 返回 | 说明 |
|---|---|---|---|
| `chat_complete` | messages、tools、reasoning 等 | Completion | 对话/补全，支持函数调用与思考模式开关 |
| `embed` | texts | List[Embedding] | 文本向量化，供知识条目入库 |
| `chat_structured` | messages、json_schema | JSON | 结构化输出（`response_format`；reasoning=on 时回退解析）|

接入实现：

| 实现类 | 供应商 | 适配要点 |
|---|---|---|
| `DeepSeekProvider` | DeepSeek | Function Calling / reasoning 思考开关 / 长上下文 / 自动上下文缓存 |
...
| 语义检索 P95 | ≤ 500ms |
| 批量提取吞吐 | reasoning=off 批量并行（deepseek-v4-flash），按队列配额流控 |
| 周扫冲突检测 | 异步后台，不影响在线请求 |
...
| 维度 | 设计 |
|---|---|
| 模型服务容错 | DeepSeek API 异常统一重试（重试 2 次），请求不中断 |
...
| 方向 | 设计 |
|---|---|
| 引擎扩展 | 模型抽象接口保留，未来可替换/新增服务商（本期仅 DeepSeek）|
...
单一供应商（DeepSeek）下，按 **reasoning 开关（思考开合）+ 自动上下文缓存 + 批量合并** 控制成本。占比示意如下（均基于单一 `deepseek-v4-flash`）：

| 使用类型 | 概览 |
|---|---|
| 对话/推理 | 在线交互调用，reasoning=off（对话）或 reasoning=on（推理）|
| 生成/简报 | 周期与页面生成，配提示词模板，reasoning=off |
| 批量提取/主题分析 | 后台批量，reasoning=off + 自动上下文缓存 |

> 具体成本预估在接入 DeepSeek 并实测后回填，本文档不固锁单一价格数字。

### 11.2 降本手段

| 手段 | 说明 |
|---|---|
| reasoning 开关 | 深度任务开思考、机械任务关思考，按需付费省的是思考 token |
| 自动上下文缓存 | 系统提示词、知识库摘要自动缓存，命中输入成本大幅下降（无需手动管理缓存前缀）|
...
| 依赖故障 | DeepSeek API 超时/限流、向量库不可用、知识源连接失败 | 重试 → 降级 → 告警，不中断整体服务 |
...
| 依赖 | 降级策略 | 用户可感知程度 |
|---|---|---|
| DeepSeek API | 交互链路：提示「暂时无法响应，请稍后重试」；异步链路：跳过本周期并记录，下周期补跑 | 低 |
...
| 可靠性 | 混沌演练：杀 worker、断向量库、模拟 DeepSeek 5xx | 数据不丢失、任务可续跑、降级生效 |
| 安全 | 越权访问、提示注入样本、敏感词输出 | 无越权、注入被拦截、敏感内容零输出 |
| 幂等 | 同一 `task_key` 重复投递 3 次 | 副作用仅发生一次 |

### 13.4 测试数据与模型 Mock

- 测试环境**禁止**调用真实 DeepSeek API，使用 Mock Provider 返回固定响应，保证测试确定性且零成本。
...
| R-04 | 成本超预算 | 成本 | 中 | 高 | 候选漏斗 + 自动缓存 + reasoning 开关 + 预算闸门 | 预算使用 > 80% |
| R-05 | 自动上下文缓存命中率低 | 成本 | 中 | 中 | 稳定前缀优先，减小高频区扰动 | 缓存命中率 < 40% |
...
| 阶段 | 周期 | 交付物 | 阶段验收 |
|---|---|---|---|
| P0 网关与数据底座 | 3 周 | ModelGateway、DeepSeekProvider、存储与向量化 | 一次模型调用全链路可追踪、可计费 |
| P1 L1+L2 对话与冲突 | 3 周 | 多轮澄清、冲突检测闭环 | 首字 ≤3s；L1 收敛轮数中位数 ≤2 |
| P2 L3+L4 助产与规划 | 2-3 周 | 认知助产、路径修正、Function Calling | 任务重复执行率 0 |
...
| OI-04 | DeepSeek 单一模型 reasoning 开关与结构化输出的精确兼容性 | 成本与质量 | 接入真实 API 后实测校准 3.1.2 |
